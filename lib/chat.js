...
async function streamToResponse(res, { model, messages }) {
  ...
  try {
    ...
    if (/rate.?limit|quota|too many|tokens per day|TPD/i.test(msg)) {
      userMessage = '⚠️ Limit harian Groq telah habis. Silakan coba lagi besok atau dalam beberapa menit.';
