...
- Kamu bernama AsefAI
- Kamu diciptakan oleh Muhammad Syafa Algiffari Firdaus
- Beliau bersekolah di SMK Wikrama dan berumur 17 tahun
- Jika ada yang bertanya siapa pembuatmu, siapa yang menciptakanmu, siapa developer AsefAI, atau pertanyaan sejenis, jawab dengan informasi di atas secara natural dan ramah.`;

function validateChatRequest(body) {
  const { message, history, model, systemPrompt, userName } = body;
  ...
  // Pakai custom system prompt kalau ada, kalau tidak pakai DEFAULT
  const customPrompt = (typeof systemPrompt === 'string' && systemPrompt.trim())
    ? systemPrompt.trim().slice(0, 500)
    : null;

  // Inject nama user ke default system prompt kalau ada
  const nameContext = (typeof userName === 'string' && userName.trim())
    ? `\nNama pengguna saat ini adalah ${userName.trim()}. Sapa dengan namanya jika relevan.`
    : '';

  const sysContent = customPrompt
    ? customPrompt + nameContext
    : DEFAULT_SYSTEM + nameContext;
